...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  componentDidUpdate (prevProps: AllWidgetProps<IMConfig>, prevState: State) {
    ...
    if (this.isParentWidgetIsController(prevProps) && this.props.inControllerUx === 'offPanel') {
  ...
  updateUrls = (options?: { enableShortUrl?: boolean, urlExcludedUrlParams?: string }): void => {
    ...
    if (enableShortUrl) {
      ...
      ShortLinkUtil.fetchShortLink(href, this.props.config?.shortenerServiceUrl).then((shortUrl) => {
        const nextSharedUrl = typeof shortUrl === 'string' ? shortUrl.trim() : ''
        this._cached.shortUrl = nextSharedUrl

        this.setState({
          sharedUrl: nextSharedUrl || href,
          isFetchingShortLink: false,
          errorInfo: null
        }) // loading
      }, (failedInfo) => {
  ...
  render () {
    ...
    return (
      <Paper shape='none' variant='flat' transparent={!(uiMode === UiMode.Popup && this.state.isPopupInController)} css={getStyle(this.props.theme,this.state.isControllerOffPanelStyle)} data-testid='share-widget' className={this.state.isControllerOffPanelStyle ? 'px-3 py-3' : ''}>
        <React.Fragment>
          {/* 1.popup mode */}
